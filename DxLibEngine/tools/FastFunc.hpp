...
// 			using BaseType::BaseType;
			...
			inline FastFunc(std::nullptr_t np) : BaseType(np) { }
			inline FastFunc(BaseType::PtrStaticFuncT mFunc) : BaseType(mFunc) { }
			template<typename X, typename Y> inline FastFunc(X* mThis, Y mFunc) : BaseType(mThis, mFunc) { }
...
#else
	template<typename T> struct MemFuncToFunc;
	template<typename TReturn, typename TThis, typename TArgs> struct MemFuncToFunc<TReturn(TThis::*)(TArgs) const> { typedef TReturn(*Type)(TArgs); };

	template<typename T> class FastFunc;
	template<typename TReturn, typename TArgs> class FastFunc<TReturn(TArgs)>
	{
	private:
		std::shared_ptr<void> storage;
		template<typename T> inline static void funcDeleter(void* mPtr) { static_cast<T*>(mPtr)->~T(); operator delete(mPtr); }
	...
	public:
		inline FastFunc() noexcept { };
		inline FastFunc(std::nullptr_t np) : BaseType(np) { }

		template<typename X, typename Y> inline FastFunc(X* mThis, Y mFunc) { }
		void operator()(TArgs) { }
	};
	#undef ENABLE_IF_CONV_TO_FUN_PTR
	#undef ENABLE_IF_NOT_CONV_TO_FUN_PTR
